...
export default function App() {
  ...
  const handleMarkDestroyed = async () => {
    ...
    try {
      ...
      setFeedback(`Batch ${selectedBatch.batch_number} marked as DESTROYED.`);
  ...
  return (
    <div style={{ maxWidth: '1000px', margin: '30px auto', fontFamily: 'system-ui, sans-serif', padding: '0 20px' }}>
      <header style={{ borderBottom: '2px solid #e5e7eb', paddingBottom: '16px', marginBottom: '24px' }}>
        ...
        <p style={{ margin: '4px 0 16px 0', color: '#6b7280' }}>Track. Verify. Prevent Re-entry.</p>
        <div style={{ display: 'flex', gap: '8px' }}>
          <button
            ...
            style={{
              ...
              borderRadius: '6px',
              cursor: 'pointer',
              background: activeTab === 'manufacturer' ? '#1f2937' : '#e5e7eb',
              color: activeTab === 'manufacturer' ? '#fff' : '#1f2937',
              fontWeight: 600
            }}
          >
          ...
          <button
            ...
            style={{
              ...
              borderRadius: '6px',
              cursor: 'pointer',
              background: activeTab === 'scanner' ? '#dc2626' : '#fee2e2',
              color: activeTab === 'scanner' ? '#fff' : '#991b1b',
              fontWeight: 600
            }}
          >
            Re-entry Scanner
      ...
      {feedback && (
        <div style={{ background: '#ecfdf5', border: '1px solid #10b981', color: '#065f46', padding: '12px', borderRadius: '6px', marginBottom: '20px' }}>
          {feedback}
        </div>
      )}
      ...
      {activeTab === 'manufacturer' ? (
        <div>
            ...
            <tbody>
              {batches.map((b) => (
                <tr key={b.id} style={{ borderBottom: '1px solid #e5e7eb' }}>
                  <td style={{ padding: '10px', fontWeight: 600 }}>{b.batch_number}</td>
                  <td style={{ padding: '10px' }}>{b.medicine_name}</td>
                  ...
                  <td style={{ padding: '10px' }}>
                    <span style={{
                      ...
                      fontSize: '12px',
                      fontWeight: 600,
                      background: b.status === 'DESTROYED' ? '#fee2e2' : '#e0e7ff',
                      color: b.status === 'DESTROYED' ? '#991b1b' : '#3730a3'
                    }}>
                      {b.status}
                    </span>
                  </td>
                  <td style={{ padding: '10px' }}>
                    {b.status === 'DISTRIBUTOR_RECEIVED' && (
                      <button onClick={() => handleVerifyReceipt(b)} disabled={loading} style={{ padding: '6px 12px' }}>
                        Confirm Receipt
                      </button>
                    )}
                    {b.status === 'MANUFACTURER_RECEIVED' && (
                      <button onClick={() => handleScheduleDestruction(b)} disabled={loading} style={{ padding: '6px 12px' }}>
                        Schedule Destruction
                      </button>
                    )}
                    {b.status === 'PENDING_DESTRUCTION' && (
                      <button onClick={() => setSelectedBatch(b)} disabled={loading} style={{ padding: '6px 12px', background: '#d97706', color: '#fff', border: 'none', borderRadius: '4px' }}>
                        Process Destruction
                      </button>
                    )}
                  </td>
          ...
          {selectedBatch && (
            <div style={{ marginTop: '24px', padding: '20px', border: '2px solid #d97706', borderRadius: '8px', background: '#fffbeb' }}>
              <h3>Destruction Processing: {selectedBatch.batch_number}</h3>
              <p>Medicine: {selectedBatch.medicine_name} | Quantity: {selectedBatch.quantity}</p>

              <div style={{ marginBottom: '12px' }}>
                <label style={{ display: 'block', marginBottom: '4px' }}>Waste Facility:</label>
                <input type="text" value={facilityId} onChange={(e) => setFacilityId(e.target.value)} style={{ padding: '6px', width: '300px' }} />
              </div>

              <div style={{ marginBottom: '12px' }}>
                <label style={{ display: 'block', marginBottom: '4px' }}>Certificate Number:</label>
                <input type="text" value={certNumber} onChange={(e) => setCertNumber(e.target.value)} style={{ padding: '6px', width: '300px' }} />
                <button onClick={handleVerifyCert} style={{ marginLeft: '8px', padding: '6px 12px' }}>Verify</button>
                {certVerified === true && <span style={{ color: 'green', marginLeft: '8px', fontWeight: 'bold' }}>✓ VERIFIED</span>}
                {certVerified === false && <span style={{ color: 'red', marginLeft: '8px', fontWeight: 'bold' }}>⚠️ MISMATCH</span>}
              </div>
              ...
              <button
                ...
                style={{
                  background: certVerified ? '#dc2626' : '#9ca3af',
                  color: '#fff',
                  padding: '10px 18px',
                  border: 'none',
                  borderRadius: '4px',
                  cursor: certVerified ? 'pointer' : 'not-allowed',
                  fontWeight: 600
                }}
              >
                Mark Batch as Destroyed
              </button>
              <button onClick={() => setSelectedBatch(null)} style={{ marginLeft: '10px', padding: '10px 18px' }}>Cancel</button>
            </div>
          )}
        </div>
      ) : (
        <div>
          ...
          <div style={{ display: 'flex', gap: '8px', marginBottom: '20px' }}>
            <input
              type="text"
              value={scanBatchNum}
              onChange={(e) => setScanBatchNum(e.target.value)}
            ...
            <button
              onClick={handleScan}
              disabled={loading}
              style={{ padding: '10px 20px', background: '#dc2626', color: '#fff', border: 'none', borderRadius: '4px', fontWeight: 600, cursor: 'pointer' }}
            >
            ...
            <div>
                    ...
                    <ul style={{ paddingLeft: '20px' }}>
                      {timeline.map((evt, idx) => (
                        <li key={idx} style={{ marginBottom: '8px' }}>
                          <strong>{evt.event_type}</strong> — {evt.location} ({new Date(evt.timestamp).toLocaleString()})
                        </li>
                      ))}
                    </ul>
                  </div>
              ...
              {scanResult.status === 'VALID' && (
                <div style={{ background: '#ecfdf5', border: '1px solid #10b981', padding: '20px', borderRadius: '8px' }}>
                  <h2 style={{ color: '#065f46', margin: 0 }}>✓ VALID BATCH</h2>
                  <p>Status: ACTIVE</p>
                </div>
              )}

              {scanResult.status === 'UNKNOWN_BATCH' && (
                <div style={{ background: '#f3f4f6', border: '1px solid #9ca3af', padding: '20px', borderRadius: '8px' }}>
                  <h2>UNKNOWN BATCH</h2>
